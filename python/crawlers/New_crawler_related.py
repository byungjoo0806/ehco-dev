...
class NewsProcessor:
    def __init__(self, celebrity: Dict[str, str]):
        ...
        self.rate_limiter = asyncio.Semaphore(3)  # Increased from 3 to 5 for larger batches
            ...
            try:
                while current_page <= max_pages:
                    ...
                    try:
                        ...
                        print(f"Successfully processed {articles_processed} articles on page {current_page}")
    ...
    async def crawl_joongang(self):
        ...
        max_irrelevant_streak = 3  # Stop after this many consecutive irrelevant articles
                ...
                while True:
                    ...
                    for idx in range(start_idx, current_count):
                        try:
                            ...
                            relevance_score = self.extract_relevance_score(response.content[0].text)
                            relevance_reason = self.extract_relevance_reason(response.content[0].text)
                            
                            print(f"Article {idx + 1} relevance score: {relevance_score}")
                            ...
                            if relevance_score >= 3.0:
                                ...
                                irrelevant_streak = 0
                                print(f"Added relevant article: {full_article['title'][:50]}...")
                            else:
                                # Article is not relevant
                                irrelevant_streak += 1
                                print(f"Skipped irrelevant article (streak: {irrelevant_streak})")
                                
                                if irrelevant_streak >= max_irrelevant_streak:
                                    print(f"Stopping crawl after {irrelevant_streak} consecutive irrelevant articles")
                                    return articles
            ...
            try:
                async with self.rate_limiter:
                    ...
                    for article in source_articles:
                        if (article and 
                            self.validate_article(article) and 
                            article['url'] not in seen_urls):
                            seen_urls.add(article['url'])
                            valid_articles.append(article)
                    
                    articles.extend(valid_articles)
                    print(f"Successfully crawled {len(valid_articles)} articles from {source}")
    ...
    def is_article_relevant(self, response: str) -> bool:
        ...
        try:
            ...
            return score >= 3.0  # Articles with score 3 or higher are considered relevant
        except Exception as e:
    ...
    def extract_headline_data(self, response: str) -> Dict[str, str]:
        ...
        try:
            ...
            headline = next((line.replace('HEADLINE:', '').strip() 
                        for line in lines if line.startswith('HEADLINE:')), "")
            subheading = next((line.replace('SUBHEADING:', '').strip() 
                            for line in lines if line.startswith('SUBHEADING:')), "")
            return {
                'headline': headline,
    ...
    def extract_category_data(self, response: str) -> Dict[str, str]:
        ...
        try:
            lines = response.split('\n')
            category = next((line.replace('CATEGORY:', '').strip() 
                            for line in lines if line.startswith('CATEGORY:')), "")
            subcategory = next((line.replace('SUBCATEGORY:', '').strip() 
    ...
    async def analyze_relevance(self, articles: List[Dict]) -> pd.DataFrame:
        ...
        for batch in self.batch_articles(articles, self.BATCH_SIZE):
            try:
                print(f"Processing batch of {len(batch)} articles for relevance analysis...")
    ...
    async def generate_headlines(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for batch in self.batch_articles(df.to_dict('records'), self.BATCH_SIZE):
            print(f"Processing batch of {len(batch)} articles for headline generation...")
            ...
            for article, response in zip(batch, responses):
                if response:
                    ...
                    articles_with_headlines.append({
                        **article,
                        **headline_data
                    })
        
        headlines_df = pd.DataFrame(articles_with_headlines)
        output_file = self.output_dir / f'{self.celebrity_id}_articles_with_headlines.csv'
    ...
    async def categorize_articles(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Create DataFrame with categorized articles
        categorized_df = pd.DataFrame(categorized_articles)
        
        # Find related articles
        final_df = await self.find_related_articles(categorized_df)
    ...
    async def upload_to_firebase(self, df: pd.DataFrame):
        ...
        # Create a URL to doc_id mapping for the entire dataset
        url_to_docid = {
            ...
            try:
                for _, row in batch_df.iterrows():
                    ...
                    if not all(field in row and pd.notna(row[field]) for field in required_fields):
                        print(f"Skipping row due to missing required fields: {row['url']}")
                        continue
                    
                    # Get doc_id for current article
                    current_doc_id = url_to_docid[row['url']]
                    doc_ref = self.db.collection('news').document(current_doc_id)
                    
                    # Convert related article URLs to doc_ids and ensure no self-reference
                    related_doc_ids = [
                        url_to_docid[url] 
                        for url in row.get('related_articles', [])
                        if url in url_to_docid and url_to_docid[url] != current_doc_id  # Exclude self-reference
                    ]
                    
                    # If no related articles after filtering, set to empty array
                    if not related_doc_ids:
                        related_doc_ids = []
                        relation_reason = ""  # Clear the reason if no related articles
                    else:
                    ...
                    doc_data = {
                        ...
                        'related_articles': related_doc_ids,
                ...
                for attempt in range(max_retries):
                    try:
                        ...
                        print(f"Successfully uploaded batch {start_idx//BATCH_SIZE + 1}")
                        break
                    except Exception as e:
    ...
    async def batch_claude_call(self, prompts: List[str]) -> List[Optional[str]]:
        ...
        async def process_single_prompt(prompt: str, attempt: int = 0, max_retries: int = 3) -> Optional[str]:
            async with rate_limit:
                try:
                    ...
                    if "rate_limit_error" in str(e):
                        ...
                        print(f"Error occurred, waiting {wait_time:.2f}s before retry...")
        ...
        for i in range(0, len(prompts), chunk_size):
            ...
            print(f"Progress: {min(i + chunk_size, total_prompts)}/{total_prompts} prompts processed")
        ...
        print(f"Batch success rate: {success_count}/{total_prompts} ({success_count/total_prompts*100:.1f}%)")
    ...
    def create_relevance_prompt(self, article: Dict) -> str:
        return f"""Analyze this article's relevance to {self.celebrity['name_eng']}.
...
REASON: (brief explanation)"""
    ...
    def create_headline_prompt(self, article: Dict) -> str:
        return f"""Create a headline and subheading for this article about {self.celebrity['name_eng']}.
Content: {article['content']}
...
async def main():
    try:
        celebrity = {"name_eng": "Han So-hee", "name_kr": "한소희"}
